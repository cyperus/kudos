...
import { emojiMap } from '~/utils/contants';

interface KudoWithRecipient extends Kudo {
  recipient: User;
}
export default function RecentBar({ kudos }: { kudos: KudoWithRecipient[] }) {
  return (
      ...
      <div className="h-full flex flex-col gap-y-10 mt-10">
        {kudos.map((kudo) => (
          <div className="h-24 w-24 relative" key={kudo.id}>
            <UserCircle profile={kudo.recipient.profile} className="h-20 w-20" />
            <div className="h-8 w-8 text-3xl bottom-2 right-4 rounded-full absolute flex justify-center items-center">{emojiMap[kudo.style?.emoji || 'THUMBSUP']}</div>
          </div>
        ))}
      </div>
    </div>
