...
import { backgroundColorMap, colorMap, emojiMap } from '~/utils/contants';
import UserCircle from './user-circle';

export default function Kudo({ profile, kudo }: { profile: Profile; kudo: Partial<IKudo> }) {
  return (
    <div className={`flex ${backgroundColorMap[kudo.style?.backgroundColor || 'RED']} p-4 rounded-xl w-full gap-x-2 relative`}>
      <div>
        <UserCircle profile={profile} className="h-16 w-6" />
      </div>
      ...
      <div className="absolute bottom-4 right-4 bg-white rounded-full h-10 w-10 flex items-center justify-center text-2xl">{emojiMap[kudo.style?.emoji || 'THUMBSUP']}</div>
    </div>
  );
}
